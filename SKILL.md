...
| `413` | `request_too_large`, `pdf_upload_too_large_for_api` | Payload too large. Split batch requests; for PDFs, use the browser/session resumable upload flow rather than retrying the raw API upload. |
...
- Browser/session JWT item PDF routes live under `/api/v1/google-drive/vaults/:vaultId/items/:itemId/pdf`, `/session`, and `/complete`. API-key agents must not call those routes; they are for the web UI and Supabase session auth.
