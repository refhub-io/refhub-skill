...
Uses a pre-issued API key for data routes. Key scope and vault restrictions bound what the agent can do. Supabase session JWT routes are account-management/browser flows, not ordinary agent runtime.
...
Skill expectation: surface the Drive URL from the response (`data.driveUrl`) so the agent can record or display it — **this is the only time the API returns it.** As of this writing, `GET /vaults/:vaultId` and `GET /vaults/:vaultId/items/:itemId` do not include the stored Drive URL anywhere (verified 2026-07 against the live API); it is not persisted onto any readable field, though a `drive_pdf_url` field exposing exactly this is in progress separately (refhub-io/.netlify#22) — check whether that has shipped before assuming this limitation still holds. This corresponds to the frontend's `drive_pdf` field, which the frontend itself reads via a direct client-side join against `publication_pdf_assets` — not through the public API. `driveUrl` is a deliberately distinct name from `pdf_url` (the publisher-hosted PDF link field on the publication) — they are unrelated fields. CLI: `refhub pdf upload --vault <vaultId> --item <itemId> --file <path.pdf>`. `publication_pdf_assets` canonical-row delete/insert behavior is an internal frontend/schema detail, not an agent API promise.
